...
Duration derives from durable `Match.startedAt` and `Match.finishedAt` (or their existing canonical equivalents). During ACTIVE play the UI may tick locally from `startedAt`; FINISHED duration is fixed. Reconnect, reload, and surrender must not reset it.
