...
- Exact inactive-corner removal remains a normal `MOVE_PAWN` transition: it emits `pawnMoved` to the physical corner followed by `pawnRemoved(INACTIVE_CORNER_EXIT)`, then persists the pawn as `OFF_BOARD`.
