...
describe('realtime contracts', () => {
  it('accepts intent-only gameplay commands and sync requests', () => {
    expect(
      gameCommandRequestSchema.parse({
        type: 'ROLL_DICE',
        matchId: 'm1',
        actionId: 'a1',
    ...
    expect(
      gameSyncRequestSchema.parse({ matchId: 'm1', stateVersion: 2, lastSequence: 3 }),
    ).toEqual({
      matchId: 'm1',
      stateVersion: 2,
      lastSequence: 3,
  ...
  it('rejects invalid transition ranges', () => {
    expect(() =>
      transitionEnvelopeSchema.parse({
        matchId: 'm1',
        transitionId: 'm1:a1',
        stateVersion: 2,
        fromSequence: 3,
