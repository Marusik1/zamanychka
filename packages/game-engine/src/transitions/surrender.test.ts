...
describe('surrender transition', () => {
  it('removes all four pawns in canonical order and keeps non-current turn state unchanged', () => {
    ...
    expect(
      result.ok && result.state.players.find((player) => player.playerId === 'p2')?.status,
    ).toBe('SURRENDERED');
    expect(
      result.ok &&
        result.state.pawns
          .filter((pawn) => pawn.playerId === 'p2')
          .every((pawn) => pawn.position.zone === 'REMOVED'),
    ).toBe(true);
  });
  ...
  it('terminalizes as LAST_ACTIVE_PLAYER when surrender leaves exactly one active player', () => {
    const state = setPlayerStatus(
      setPlayerStatus(baseState(), 'p2', 'SURRENDERED'),
      'p3',
      'SURRENDERED',
    );
    const result = transition(
    ...
    expect(
      result.ok && result.state.players.find((player) => player.playerId === 'p1')?.status,
    ).toBe('SURRENDERED');
    expect(
      result.ok && result.state.players.find((player) => player.playerId === 'p4')?.status,
    ).toBe('FINISHED');
  });
