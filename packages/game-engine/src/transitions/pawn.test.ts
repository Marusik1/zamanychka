...
function findPerimeterProgress(color: GameState['players'][number]['color'], coord: { row: number; col: number }): number | null {
...
describe('pawn transitions', () => {
  it('enters a pawn on six, emits extraRollGranted, and keeps the same player', () => {
    ...
    const result = transition(state, { type: 'ENTER_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 6 });
    ...
    expect(result.ok && result.state.pawns.find((pawn) => pawn.pawnId === 'p1-pawn-1')?.position).toEqual({ zone: 'PERIMETER', progress: 0 });
  });

  it('rejects enter on an occupied corner and does not mutate state', () => {
    const occupied = setPawnPosition(moveIntoActionState(baseState(), 6), 'p1-pawn-2', { zone: 'PERIMETER', progress: 0 } as const);
    const snapshot = structuredClone(occupied);
    const result = transition(occupied, { type: 'ENTER_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 6 });
  ...
  it('moves normally on 1-5 with exact turn change and capture', () => {
    ...
    const result = transition(state, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 2 });
  ...
  it('moves on six and grants an explicit extra roll', () => {
    ...
    const result = transition(state, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 6 });
  ...
  it('rejects own destination blockers and intermediate blockers', () => {
    ...
    const ownBlocked = setPawnPosition(ownBlocker, 'p1-pawn-2', { zone: 'PERIMETER', progress: 3 } as const);
    const ownResult = transition(ownBlocked, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 2 });
    ...
    const intermediateBlocked = setPawnPosition(ownBlocker, 'p2-pawn-1', { zone: 'PERIMETER', progress: intermediateProgress! } as const);
    const intermediateResult = transition(intermediateBlocked, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 2 });
  ...
  it('captures an ordinary opponent perimeter destination and returns the opponent to OFF_BOARD', () => {
    ...
    const withOpponent = setPawnPosition(state, 'p2-pawn-1', { zone: 'PERIMETER', progress: opponentProgress! } as const);
    const result = transition(withOpponent, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 2 });
    ...
    expect(result.ok && result.state.pawns.find((pawn) => pawn.pawnId === 'p2-pawn-1')?.position).toEqual({ zone: 'OFF_BOARD' });
  });
  ...
  it('enters HOME from the perimeter, emits pawnEnteredHome once, and supports exact landing up to HOME(3)', () => {
    ...
    const result = transition(state, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 3 });
    ...
    expect(result.ok && result.state.pawns.find((pawn) => pawn.pawnId === 'p1-pawn-1')?.position).toEqual({ zone: 'HOME', homeIndex: 1 });
  });
  ...
  it('rejects HOME overshoot and rejects pass-through when HOME(0) is occupied', () => {
    ...
    const overshootResult = transition(overshoot, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 6 });
    ...
    const blocked = setPawnPosition(occupiedCorner, 'p2-pawn-1', { zone: 'PERIMETER', progress: blockerProgress! } as const);
    const blockedResult = transition(blocked, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 3 });
  ...
  it('captures on the exact HOME(0) destination when an opponent occupies that physical corner', () => {
    ...
    const withOpponent = setPawnPosition(state, 'p2-pawn-1', { zone: 'PERIMETER', progress: opponentProgress! } as const);
    const result = transition(withOpponent, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 1 });
  ...
  it('terminalizes immediately when the fourth pawn completes HOME and emits gameWon last', () => {
    ...
    const result = transition(state, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-4', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 1 });
  ...
  it('is deterministic and leaves the input state unchanged', () => {
    ...
    const first = transition(state, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 2 });
    const second = transition(state, { type: 'MOVE_PAWN', actorPlayerId: 'p1', matchId: 'm1', pawnId: 'p1-pawn-1', expectedStateVersion: 0 }, { actorPlayerId: 'p1', diceValue: 2 });
